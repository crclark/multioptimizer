...
dominates x y = U.any id (U.zipWith (>) x y) && U.all id (U.zipWith (>=) x y)

domOrdering :: U.Vector Double -> U.Vector Double -> Ordering
domOrdering x y = case (dominates x y, dominates y x) of
...
scaleObjs vs = if V.length vs < 2
  then V.map (U.map (const 1.0)) vs
    ...
    let maxes = V.foldr1 (U.zipWith max) vs
...
crowdingDists Frontier {..} = foldr1
  (V.zipWith (+))
  (map mthCrowdingDists [0 .. fromIntegral numObjs - 1])
 where
  mthCrowdingDists :: Int -> V.Vector Double
  mthCrowdingDists m =
    let n = V.length frontier
        sorted =
          V.modify (Merge.sortBy (comparing (U.! m))) (V.map snd frontier)
...
shrinkToSize maxItems f@Frontier {..} =
  if V.length frontier > fromIntegral maxItems
    then
      let sortedByCrowding = V.modify
            (Merge.sortBy (comparing $ negate . snd))
            (V.zip frontier (crowdingDists f))
      in
        f
          { frontier = V.take (fromIntegral maxItems)
